...
async def check_missing_v2():
    async with AsyncSession(engine) as session:
        ...
        for u in unparsed:
            # Removed created_at assumption
            print(f"UNPARSED [ID:{u.id}] Err: {u.error_message} | Content: {u.raw_line[:150]}")
        ...
        for r in raids:
            print(f"RAID [ID:{r.id}] Time:{r.timestamp} | Atk: {r.attacker_name} | Success: {r.is_success}")

if __name__ == "__main__":
    asyncio.run(check_missing_v2())
