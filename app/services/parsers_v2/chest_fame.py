...
class ChestFameParserV2:
    ...
    REGEX_CHEST = re.compile(r"(?P<timestamp>[\d\.-]+): Chest \(entity id: (?P<eid>\d+)\) ownership (?:(?P<action>claimed)\. Owner|(?P<action_chg>changed).*?New owner): (?P<steam_id>\d+).*Location: X=(?P<x>[\d\.-]+) Y=(?P<y>[\d\.-]+) Z=(?P<z>[\d\.-]+)")
    
    # 2. FAME
    # 2025.12.21-16.10.51: Player BOT_Oblivion(76561199817469068) was awarded 20.000000 fame points for Kill.
    REGEX_FAME = re.compile(r"(?P<timestamp>[\d\.-]+): Player (?P<name>.*?)\((?P<steam_id>\d+)\) was awarded (?P<amount>[\d\.-]+) fame points for (?P<reason>.*)")
